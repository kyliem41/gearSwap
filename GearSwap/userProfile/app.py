...
def lambda_handler(event, context):
    ...
    # elif resource_path == '/userProfile/{Id}/image':
    #     if http_method == 'PUT':
    #         return updateProfileImage(event, context)
    #     elif http_method == 'GET':
    #         return getProfileImage(event, context)
...
# def updateProfileImage(event, context):
#     try:
#         user_id = event['pathParameters']['Id']
...
#         if not content_type:
#             return cors_response(400, {'error': 'Content-Type header is required'})
...
#                 return cors_response(400, {'error': f'File size exceeds maximum limit of {MAX_FILE_SIZE/1024/1024}MB'})

#             # Validate image format
#             if content_type == 'image/png' and not image_data.startswith(b'\x89PNG\r\n\x1a\n'):
#                 return cors_response(400, {'error': 'Invalid PNG image data'})
#             elif content_type == 'image/jpeg' and not image_data.startswith(b'\xff\xd8'):
#                 return cors_response(400, {'error': 'Invalid JPEG image data'})
...
#             return cors_response(200, {'message': 'Profile picture updated successfully'})
...
# def getProfileImage(event, context):
#     try:
#         user_id = event['pathParameters']['Id']
...
#             with conn.cursor(cursor_factory=RealDictCursor) as cursor:
#                 cursor.execute("""
#                     SELECT profilePicture, profile_picture_content_type
#                     FROM userProfile
#                     WHERE userId = %s
#                 """, (user_id,))
...
#                 if not result or not result['profilePicture']:
#                     return cors_response(404, {'error': 'Profile picture not found'})
